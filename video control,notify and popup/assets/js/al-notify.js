var bodywapper = document.getElementById('show_notify'); //show Wrapper  
var x = 0;
let alNotifyArray = [ //array with object all notify data 
    {
...
setInterval(function() { // notify show after count a time 
    ...
    if (x < alNotifyArray.length) {
        ...
        setTimeout(socialProfNotifyAnimation, 100); // animation show after show 
        function socialProfNotifyAnimation(){
            document.querySelector('.custom-social-proof').style.left = '1%';
        }   
        
      
    }

    if(x == alNotifyArray.length){ // repeat loop
        x= 0
    }
    else{
        x++;
    }
    document.querySelector('.custom-close').addEventListener('click',hidenotify); // close button event

}, 5000);


function hidenotify(){ //default css on parent 
    document.querySelector('.custom-social-proof').style.left='-100%';
}
